-- this code is shared with multiple wikis, please edit the module in https://github.com/utgwiki/sync instead of here.
...
--- @return string html
function methodtable.renderImage( self, filename, imageClass )
	...
	-- Add custom image class if provided
	if imageClass then
		html:addClass(imageClass)
	end

	html:wikitext( string.format( '[[File:%s|400px]]', filename ) )
...
function methodtable.renderInfobox( self, innerHtml, snippetText )
	...
	if self.modules.FloatingUI then
	    local floating = tostring(self.modules.FloatingUI.load(frame) or '')
	    output = floating .. tostring(output)
	end
...
function InfoboxNeue.fromArgs( frame )
    ...
    if args['image'] then
	    instance:renderImage( args['image'], args['imageClass'] )
    end
    ...
    for i = 1, 50, 1 do
        ...
        -- handle dimensions (splits into a new top-level section)
    ...
    -- final render in exact order of the plan
	...
	-- used for utg wiki only for notebooklm integration
	if args['notebook'] then
        instance:renderSection( {
            class = 'infobox__section--actions infobox__section--hasBackground',
            content = instance:renderItem( {
                icon = args['notebook-image'] or 'WikimediaUI-Journal.svg',
                data = args['notebook-title'] or 'Related notebook',
                desc = args['notebook-desc'] or 'Powered by NotebookLM',
                link = args['notebook']
            } )
        } )
    end
